...
pub(crate) fn new_ns_loader() -> BTreeMap<IriBuf, RemoteDocument> {
    ...
    let iri = IriBuf::new("https://www.w3.org/ns/credentials/examples/v2".to_string()).unwrap();
    map.insert(
        iri.clone(),
        RemoteDocument::new(
            Some(iri),
            Some("application/ld+json".parse().unwrap()),
            Value::parse_str(WWW_W3_ORG_NS_CREDENTIALS_EXAMPLES_V2)
                .unwrap()
                .0,
        ),
    );
...
const WWW_W3_ORG_NS_CREDENTIALS_EXAMPLES_V2: &str = r#"{
  "@context": {
